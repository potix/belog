...
func TestDefaultLoggerChange(t *testing.T) {
	...
	formatter.SetLayout("%(dateTime) [%(logLevel):%(logLevelNum)] (%(pid)) %(hostname) %(programCounter) %(loggerName) %(fileName) %(shortFileName) %(lineNum) %(message)")
...
func TestChangeLogger(t *testing.T) {
	...
	formatter.SetLayout("%(dateTime) [%(logLevel):%(logLevelNum)] (%(pid)) %(hostname) %(loggerName) %(shortFileName) %(shortFileName) %(lineNum) %(message)")
